...
package com.morcha4.frame.preloader
{
...
//			}
			...
			m.createGradientBox(this._stageWidth,this._stageHeight,0,0,0);
			trace(_stageWidth+"_"+_stageHeight);
			this.graphics.beginGradientFill(GradientType.RADIAL,[0x3c4a59,0x161a20],[1,1],[0,255],m,SpreadMethod.PAD);
			this.graphics.drawRect(0,0,this._stageWidth,this._stageHeight);
			this.graphics.endFill();
			imageControl = new flash.display.Loader();
			imageControl.contentLoaderInfo.addEventListener(Event.COMPLETE, loader_completeHandler);
			imageControl.load(new URLRequest("loader.swf"));
		}
		var imageControl:Loader;
		private function loader_completeHandler(e:Event):void{
			addChild(imageControl);
			addChild(textf);
			textf.text="";
			textf.x=this._stageWidth/2-45;
			textf.y=this._stageHeight/2+70;
			textf.textColor=0xffffff;
			imageControl.x = this._stageWidth/2-260;
			imageControl.y = this._stageHeight/2-200;
		}
		...
		{
			if(completed<total){
				textf.text=Math.round(Math.min(completed / total, 1)*50)+"%";
			}
//			if (!_barFrameSprite)
//				return;
//			
...
//				_downloadComplete = true
		...
		{
			textf.text=(Math.round((Math.min(completed / total, 1))*20)+80)+"%";
...
//		}
		...
		{
			textf.text=Math.round(Math.min(event.bytesLoaded / event.bytesTotal, 1)*(30/event.rslTotal)+event.rslIndex*(30/event.rslTotal)+50)+"%"
		}
...
//			setTimeout(function(){dispatchEvent(new Event(Event.COMPLETE))},4000); 
		}
...
import com.morcha4.frame.preloader.Preloader;
...
class ErrorField extends Sprite
{
	 ...
	 */ 
	public function ErrorField(downloadProgressBar:Preloader)
	{
		super();
